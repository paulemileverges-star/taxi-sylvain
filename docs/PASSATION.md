...
| 19 sept. | Suppression du compte chauffeur : « automatique, pas besoin de vérifier quoi que ce soit » | Fait : le chauffeur supprime son compte dans l’app ou sur `/suppression-compte`, sans vérification de redevance ni de course en cours ; ses courses non terminées repartent chez le Dispatch (alerte), le client qui suit sa course voit « en attente », et les courses retirées sortent de l’agenda du chauffeur (courriel d’annulation dès que Brevo est branché). Pages légales mises à jour. Les APK installés ont déjà le formulaire : il marche dès la mise en ligne du serveur |
