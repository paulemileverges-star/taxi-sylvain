...
| Expo Starter ou 1er octobre | activer le forfait sur l'équipe `taxisylvains-team` | recompiler les deux APK (1.2.0 compilés le 20 septembre au matin). **Depuis, cinq changements n'existent que sur le web tant qu'on ne recompile pas** : écran du code de confirmation, sélecteur de date natif (app client), canal Android « urgence », son en mode silencieux iPhone, réabonnement au suivi après coupure. Recompilation seulement avec l'accord du propriétaire |
...
1. **Suppression de compte** : **en ligne depuis le 19 septembre** pour les clients et les chauffeurs (app et page web `/suppression-compte`). **Décision du propriétaire (19 septembre) : un chauffeur supprime son compte automatiquement, sans aucune vérification** (ni redevance réglée, ni course en cours) ; ses courses non terminées repartent chez le Dispatch, qui reçoit une alerte. Conséquence acceptée : ses récapitulatifs hebdomadaires sont effacés et ses courses ne sont plus reliées à son nom. Règle écrite dans `backend/src/lib/accountDeletion.js` (`courseEnCoursBloque`) et testée. Le client, lui, reste bloqué tant qu'une course est acceptée ou en cours.
2. **Politique de confidentialité et conditions d'utilisation** : **publiées le 19 septembre** sur `api.taxisylvain.ca/confidentialite` et `/conditions`, textes vérifiés contre le code. À compléter par le propriétaire : adresse postale, raison sociale, courriel sur taxisylvain.ca quand il existera ; relecture par un juriste.
...
9. **Recompiler les deux APK** (accord du propriétaire) : voir la ligne Expo du tableau ci-dessus pour ce qui manque aux téléphones. `@react-native-community/datetimepicker` a été ajouté à l'app client : c'est une dépendance native, l'ancien APK ne la contient pas.
...
| 20 sept. (reprise, après-midi) | **Mise en ligne de la reprise** | Serveur : en ligne le 20 septembre au soir (migration appliquée, journaux « Courriels : brevo », « Notifications web (Web Push) : actives »), vérifié par `scripts/verifier-mise-en-ligne.mjs`. **Sites web : PAS encore redéployés.** La CLI Vercel s'est retrouvée déconnectée sur le PC (« No existing credentials found ») ; le propriétaire doit faire `vercel login`, puis les trois commandes du § 6 (dispatch-web, driver-app, client-app, puis `vercel alias set`), puis relancer le script de vérification. D'ici là, un NOUVEAU compte créé depuis l'ancienne version web reçoit son code par courriel mais n'a pas l'écran pour le saisir : le Dispatch peut le confirmer à sa place (bouton sur la fiche, après redéploiement de la console) ou la personne attend la nouvelle version. Les comptes existants ne sont pas concernés |
