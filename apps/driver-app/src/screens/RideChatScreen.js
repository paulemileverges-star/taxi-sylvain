...
export default function RideChatScreen({ rideId, onBack }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.title}>Message au client</Text>
      </View>
      <ScrollView ref={scrollRef} style={{ flex: 1, paddingHorizontal: 16 }} onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
        {messages.map((m) => (
          <View
            key={m.id}
            style={[styles.bubble, m.sender.role === "DRIVER" ? styles.bubbleMine : styles.bubbleTheirs]}
          >
